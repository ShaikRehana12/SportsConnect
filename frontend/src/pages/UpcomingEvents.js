    ...
    <div className="min-h-screen bg-slate-50 font-sans pt-24 pb-16 px-6 md:px-12 relative overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          <div className="bg-white border border-slate-200/60 px-5 py-3 rounded-2xl shadow-sm text-left md:text-right">
            ...
            <p className="text-xs font-black text-cyan-600 uppercase tracking-tight">{registeredMatches.length} Slots Secured</p>
          </div>
        </div>
        ...
        {registeredMatches.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {registeredMatches.map((item) => {
              ...
              // 🕒 DYNAMIC CALCULATIONS TO IDENTIFY PAST TOURNAMENTS
              const today = new Date().setHours(0, 0, 0, 0);
              ...
              return (
                <motion.div
                  key={item._id}
                  whileHover={!isExpired ? { y: -3 } : {}}
                  className={`bg-white rounded-[32px] border border-slate-200/60 shadow-sm overflow-hidden flex flex-col justify-between group relative transition-all ${
                    isExpired ? "opacity-65 grayscale saturate-50 select-none bg-slate-50/50" : ""
                  }`}
                >
                  {/* Diagonal Corner Ribbon to instantly label Inactive State */}
                  {isExpired && (
                    <div className="absolute top-3 right-[-35px] bg-slate-500 text-white text-[8px] font-black uppercase tracking-widest py-1.5 w-32 text-center rotate-45 z-10 shadow-sm">
                      Past
                    </div>
                  )}
                  ...
                  <div>
                    <div className="h-44 w-full bg-slate-900 relative overflow-hidden border-b border-slate-50">
                      ...
                      <span className={`absolute top-4 left-4 font-black text-[9px] px-2.5 py-1 uppercase rounded-lg tracking-widest shadow-sm ${
                        isExpired ? "bg-slate-600 text-white" : "bg-cyan-600 text-white"
                      }`}>
                    ...
                    <div className="p-6 space-y-4">
                      <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-wider">
                        <span className={isExpired ? "text-slate-400 font-bold" : "text-cyan-600"}>
                          {isExpired ? "🚫 Inactive Pass" : "Verified Pass Entry"}
                        </span>
                        <span className="text-slate-400">{enrolledCount}/{maxLimit} Attendees</span>
                      </div>

                      <h2 className={`text-base font-black uppercase italic tracking-tight leading-snug transition-colors line-clamp-1 ${
                        isExpired ? "text-slate-400 line-through" : "text-slate-900 group-hover:text-cyan-600"
                      }`}>
                        {item.title}
                      </h2>
                      ...
                      <div className="space-y-2 border-t border-slate-50 pt-3 text-slate-500 text-xs font-semibold">
                        <div className="flex items-center gap-2">
                          <Calendar className={`w-4 h-4 flex-shrink-0 ${isExpired ? 'text-slate-400' : 'text-cyan-500'}`} />
                          <span className={isExpired ? "text-slate-400 line-through" : ""}>
                            {item.date || "Upcoming Date"} {item.time ? `@ ${item.time}` : ""}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <MapPin className={`w-4 h-4 flex-shrink-0 ${isExpired ? 'text-slate-400' : 'text-cyan-500'}`} />
                  ...
                  <div className="p-6 pt-0 flex gap-2">
                    <button
                      ...
                      className={`flex-1 py-3.5 font-black text-[10px] uppercase tracking-widest rounded-2xl flex items-center justify-center gap-1 transition-all border-none outline-none cursor-pointer ${
                        isExpired ? "bg-slate-300 text-slate-600 hover:bg-slate-400" : "bg-slate-900 text-white hover:bg-cyan-600"
                      }`}
                    >
                      {isExpired ? "View Pass History" : "View Ticket Details"} <Sparkles className="w-3.5 h-3.5 ml-1" />
                    </button>
                    
                    <button
                      disabled={isExpired}
                      onClick={() => handleLeaveAction(item._id, item.date)}
                      className={`p-3.5 rounded-2xl bg-white border transition-all flex items-center justify-center shadow-sm ${
                        isExpired 
                          ? "border-slate-100 text-slate-300 cursor-not-allowed opacity-40 shadow-none" 
                          : "border-rose-100 text-rose-500 hover:bg-rose-50 hover:text-rose-600 cursor-pointer"
                      }`}
                      title={isExpired ? "Expired passes cannot be dropped" : "Resign Spot / Drop Ticket"}
                    >
      ...
      <AnimatePresence>
        {isTicketModalOpen && selectedTicket && (() => {
          const isModalTicketExpired = new Date().setHours(0,0,0,0) > new Date(selectedTicket.date).setHours(0,0,0,0);
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                ...
                <div className={`p-6 pt-8 pb-14 relative text-center text-white ${isModalTicketExpired ? 'bg-slate-700' : 'bg-slate-900'}`}>
                  <button 
                    onClick={() => setIsTicketModalOpen(false)}
                  ...
                  <div className={`inline-flex items-center gap-1.5 font-black text-[9px] uppercase tracking-[0.2em] px-3 py-1.5 rounded-full mb-3 border ${
                    isModalTicketExpired ? 'bg-slate-800/40 text-slate-400 border-slate-600' : 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30'
                  }`}>
                    <ShieldCheck className={`w-3 h-3 ${isModalTicketExpired ? 'text-slate-400' : 'text-cyan-400'}`} /> 
                    {isModalTicketExpired ? 'Historical Entry Reference' : 'Authorized Entry Pass'}
                  </div>
                  <h3 className={`text-xl font-black uppercase italic tracking-tight line-clamp-1 ${isModalTicketExpired ? 'text-slate-400 line-through' : ''}`}>
                ...
                <div className="bg-white p-6 space-y-6 flex-1 text-center">
                  ...
                  {/* QR Code Box Asset with dynamic Expired overlay stamp */}
                  <div className="w-36 h-36 mx-auto bg-slate-50 border border-slate-100 p-3 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden group">
                    <QrCode className={`w-full h-full text-slate-900 stroke-[1.25] transition-opacity ${isModalTicketExpired ? "opacity-15 select-none" : ""}`} />
                    
                    {isModalTicketExpired ? (
                      <div className="absolute inset-0 flex items-center justify-center rotate-[-12deg]">
                        <span className="border-4 border-rose-500 text-rose-500 font-[1000] text-[11px] uppercase tracking-widest px-2.5 py-1 rounded-xl bg-white/95 shadow-md">
                          VOIDED / PAST
                        </span>
                      </div>
                    ) : (
                  ...
                  <div className="grid grid-cols-2 gap-4 text-left border border-slate-100 p-4 rounded-2xl bg-slate-50/50">
                    ...
                    <div className="col-span-2 border-t border-slate-100 pt-2 mt-1">
                      ...
                      <div className="flex items-center gap-1 mt-0.5 text-slate-600 text-xs font-semibold">
                        <MapPin className={`w-3.5 h-3.5 flex-shrink-0 ${isModalTicketExpired ? 'text-slate-400' : 'text-cyan-500'}`} />
                    ...
                    <div className="col-span-2 border-t border-slate-100 pt-2">
                      ...
                      <div className="flex items-center gap-1 mt-0.5 text-slate-600 text-xs font-semibold">
                        <Calendar className={`w-3.5 h-3.5 flex-shrink-0 ${isModalTicketExpired ? 'text-slate-400' : 'text-cyan-500'}`} />
                        <span className={isModalTicketExpired ? 'text-slate-400 line-through' : ''}>
                          {selectedTicket.date || "Upcoming"} {selectedTicket.time ? `@ ${selectedTicket.time}` : ""}
                        </span>
                      </div>
                    </div>
                  </div>
